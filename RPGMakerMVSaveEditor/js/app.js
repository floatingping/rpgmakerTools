...
class MVSaveEditor {
    ...
    exportSaveData(fileName) {
        const content = LZString.compressToBase64(JsonEx.stringify(this._saveData))
        this._downloadAsFile(`${this._getTimeStamp()}_${fileName}`, content);
    }
}
...
const mvSaveEditor = new MVSaveEditor();
...
const vm = new Vue({
    ...
    methods: {
        loadSaveFile: function (fileName, file) {
            this._saveFileName = fileName;

            const data = JsonEx.parse((LZString.decompressFromBase64(file)));

            mvSaveEditor.loadSave(data);
        ...
        exportSaveData: function () {
            ...
            this.actors.forEach((actor, actorId) => {
                if (!actor) return;
                mvSaveEditor.setActorProps(actorId, {
                    name: actor.name,
                ...
                mvSaveEditor.setActorParamPlus(actorId, {
                    mhp: actor.plusMHP,
                    mmp: actor.plusMMP,
                    atk: actor.plusAtk,
                    def: actor.plusDef,
            ...
            mvSaveEditor.setGold(this.gold);
            mvSaveEditor.setWeapons(this.toObj(this.weaponsJson));
            mvSaveEditor.setArmors(this.toObj(this.armorsJson));
            mvSaveEditor.setItems(this.toObj(this.itemsJson));

            return mvSaveEditor.exportSaveData(this._saveFileName);
        }
    }
});
...
document.getElementById("saveFile").addEventListener("change", function (event) {
    ...
    if (file) {
        ...
        reader.onload = function (e) {
            vm.loadSaveFile(file.name, e.target.result);
        };
